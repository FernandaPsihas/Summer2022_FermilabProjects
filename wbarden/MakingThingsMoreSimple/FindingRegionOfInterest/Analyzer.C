...
void Analyzer(string input){
  ...
  float wiremap[100][300]; //wires//ticks
  fTree->Branch("wiremap",wiremap, "wiremap[100][300]/F");
...
//iterate over all wires and ticks
    for(int i = 0; i < 100; i++)
      for(int j = 0; j < 300; j++)
	wiremap[i][j] = 0;
    ...
    for(auto edep : simedeps){
      //all particles produced electrons, photons, etc.
    ...
    for(int h = 0; h < gaus_hits.size(); h++){
      ...
      //ignore anything not on plane 2 aka collection plane
...
//ensure particles are matched to a hit
...
//iterate over number of hits
	reco_N_hit++;
//get value for average tick and channel on wire
	avgtick += hit.PeakTime();
	avgwire += hit.Channel();
	
      }//iterate over truth matched hits
    ...
    for(int w = 0; w < all_wires.size(); w++){
      ...
      if(all_wires[w].Channel() < int(avgwire-24)) continue;
      if(all_wires[w].Channel() > int(avgwire+25)) continue;
	...
	if(s < int(avgtick-124)) continue; 
	if(s > int(avgtick+125)) continue; 
	       
	if(it_w > 50-1) std::cout << "too many wires! " << it_w << std::endl;
	if(it_t > 250-1) std::cout << "too many ticks! " << it_t << std::endl;
	
	wiremap[it_w][it_t] = signals[s];	  
	testfile << wiremap[it_w][it_t] << ",";
	it_t++;
      }//iterate over ticks
      it_w++;
    }//iterate over wires
    testfile<< true_E << true_x << "] \n";
    fTree->Fill();
